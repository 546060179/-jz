...
// 效果预设映射
const EFFECT_PRESETS: Record<string, { effects: string; description: string }> = {
...
// Timing tokens
const TIMING_MAP: Record<string, { ms: number; label: string }> = {
...
// Intent defaults
const INTENT_MAP: Record<string, { timing: string; easing: string }> = {
...
const ANDROID_PRESET: Record<string, string> = {
...
const IOS_PRESET: Record<string, string> = {
...
function generateCode(config: {
  ...
  if (intent) reactProps += ` intent="${intent}"`;
  if (customDuration) reactProps += ` duration={${customDuration}}`;
  else if (timing !== 't3') reactProps += ` timing="${timing}"`;
  if (delay > 0) reactProps += ` delay={${delay}}`;
...
<Motion ${reactProps}>
  ...
  let androidOptions = '';
  if (intent) androidOptions += `    options = FadeOptions(intent = MotionIntent.${intent.toUpperCase()}),\n`;
  if (customDuration) androidOptions += `    options = FadeOptions(duration = ${customDuration}L),\n`;
  if (delay > 0) androidOptions += `    options = FadeOptions(delay = ${delay}L),\n`;
...
animator.start(
  ...
  let iosOptions: string[] = [];
  if (intent) iosOptions.push(`intent: .${intent}`);
  if (customDuration) iosOptions.push(`duration: ${customDuration}`);
  if (delay > 0) iosOptions.push(`delay: ${delay}`);
