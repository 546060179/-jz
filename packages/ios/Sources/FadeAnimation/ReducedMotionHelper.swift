...
enum ReducedMotionHelper {
    ...
    static let reducedMaxDurationMs: Int = TimingScale.t1.durationMs // 100ms

    /// 全局动效级别覆盖，nil 表示跟随系统偏好
    private static var globalMotionLevel: MotionLevel?

    /// 设置全局动效级别，传入 nil 恢复为跟随系统偏好
    static func setMotionLevel(_ level: MotionLevel?) {
        globalMotionLevel = level
    }

    /// 获取当前全局动效级别设置
    static func getMotionLevel() -> MotionLevel? {
